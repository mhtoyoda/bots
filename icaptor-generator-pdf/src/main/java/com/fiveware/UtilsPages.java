...
public class UtilsPages {
    ...
    public static PageIterator forEach(String path, int pageNumber) throws IOException {
        ObjectExtractor oe = null;
            PDDocument document = PDDocument
                    .load(new File(path));
            oe = new ObjectExtractor(document);
    ...
    public static String loadCsv(String path) throws IOException {
        ...
        CSVParser parse = CSVParser.parse(new File(path), Charset.forName("utf-8"), CSVFormat.EXCEL);
